...
@RestController
@RequestMapping("/api/gobierno")
@CrossOrigin(origins = "http://localhost:3000")
public class GobiernoControlador {

    private final IVentanillaServicio ventanillaServicio;

    public GobiernoControlador(IVentanillaServicio ventanillaServicio) {
        this.ventanillaServicio = ventanillaServicio;
    }

    @GetMapping("/formularios")
    @PreAuthorize("hasRole('ADMIN') or hasRole('GOBIERNO')")
    public ResponseEntity<List<Ventanilla>> obtenerFormulariosGobierno() {
        try {
            List<Ventanilla> formularios = ventanillaServicio.obtenerFormulariosPorDependencia("Gobierno");
