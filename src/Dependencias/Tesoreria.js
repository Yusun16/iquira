...
export default function Tesoreria() {
  ...
  const FORMULARIOS_API = 'http://localhost:8080/api/tesoreria/formularios';
  const DOCUMENTOS_API = 'http://localhost:8080/api/tesoreria/documento';
  ...
  useEffect(() => {
    ...
    const fetchFormularios = async () => {
      try {
        ...
        setFormularios(response.data.filter((item) => item.dependencia === 'Tesorería'));
      } catch (error) {
