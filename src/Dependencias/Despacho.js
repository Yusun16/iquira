...
export default function Despacho() {
  ...
  const FORMULARIOS_API = 'http://localhost:8080/api/despacho/formularios';
  const DOCUMENTOS_API = 'http://localhost:8080/api/despacho/documento';
  ...
  useEffect(() => {
    const fetchFormularios = async () => {
      try {
        ...
        setFormularios(response.data.filter((item) => item.dependencia === 'Despacho'));
      } catch (error) {
