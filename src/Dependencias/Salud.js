...
export default function Salud() {
  ...
  const FORMULARIOS_API = 'http://localhost:8080/api/salud/formularios';
  const DOCUMENTOS_API = 'http://localhost:8080/api/salud/documento';
  ...
  useEffect(() => {
    ...
    const fetchFormularios = async () => {
      try {
        ...
        setFormularios(response.data.filter((item) => item.dependencia === 'direccion'));
      } catch (error) {
          ...
          >
            <div style={{ width: '250px', padding: '20px' }}>
              <Typography variant="h6" style={{ marginBottom: '10px' }}>
                Menú de Dirección de local de salud
              </Typography>
              <CustomFormControl>
